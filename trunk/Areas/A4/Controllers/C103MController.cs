...
namespace EECOnline.Areas.A4.Controllers
{
    public class C103MController : BaseController
    {
        ...
        public ActionResult Index(C103MFormModel model)
        {
            ModelState.Clear();
            A4DAO dao = new A4DAO();
            model.Grid = dao.QueryC103M_All(model);
        ...
        public ActionResult Export(C103MFormModel model)
        {
            ...
            else
            {
                ...
                ExcelWorksheet objSheet = objExcel.Workbook.Worksheets.Add("登入類別統計表");
                // 一些暫存變數
                var iRow = 1;
                var iCol = 1;
                // 產生 標題列 (固定欄位)
                ...
                return File(objExcel.GetAsByteArray(), HelperUtil.XLSXContentType, "登入類別統計表.xlsx");
            }
            return rtn;
        }
    }
