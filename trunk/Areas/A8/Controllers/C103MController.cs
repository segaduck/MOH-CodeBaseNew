...
namespace EECOnline.Areas.A8.Controllers
{
    public class C103MController : BaseController
    {
        ...
        public ActionResult Index(C103MFormModel form)
        {
            ...
            {
                if (HelperUtil.TransTwLongToDateTime(form.apy_time_st) > ((DateTime)HelperUtil.TransTwLongToDateTime(form.apy_time_ed)).AddDays(-1))
                {
                    ModelState.AddModelError("date", "寄信日期起不得大於寄信日期迄 。");
                }
            }
