# 011006 專科社會工作師證書換發（更名或污損） - 完整技術文件
...
| Controller   | `ES/Controllers/Apply_011006Controller.cs` (629 行)                |
| ViewModel    | `ES/Models/ViewModels/Apply_011006ViewModel.cs` (1,231 行)         |
| Entity Model | `ES/Models/Entities/Apply_011006Model.cs` (163 行)                 |
| DAO          | `ES/DataLayers/ApplyDAO.cs` (AppendApply011006, UpdateApply011006) |
| 資料表       | APPLY, APPLY_011006, APPLY_FILE, APPLY_LOG, APPLY_NOTICE           |

---

## 服務特色對照表

| 特色項目          | 011001 執業執照申請 | 011005 證書補發(遺失) | 011006 證書換發(更名/污損) |
| ----------------- | ------------------- | --------------------- | -------------------------- |
| 申請期間控制      | ❌                  | ❌                    | ❌                         |
| 前導說明頁        | ✅                  | ✅                    | ✅                         |
| 會員資料自動帶入  | ✅                  | ✅                    | ✅                         |
...
- 申請類別為「更名」(APPLY_TYPE = "3") 時：必須上傳戶籍謄本
...
```mermaid
graph TB
    A[使用者] --> B[Apply_011006Controller]
    B --> C[ApplyDAO]
    C --> D[APPLY 主表]
    C --> E[APPLY_011006 明細表]
    C --> F[APPLY_FILE 檔案表]
    C --> G[APPLY_LOG 歷程表]
    B --> H[Email Service]
    H --> I[SMTP Server]
...
| 欄位名稱    | 資料型別     | 說明          | 備註                 |
| ----------- | ------------ | ------------- | -------------------- |
| APP_ID      | varchar(20)  | 案件編號 (PK) | 格式：011006YYYYMMDD |
| SRV_ID      | varchar(10)  | 服務代碼      | 固定值：011006       |
| SRC_SRV_ID  | varchar(10)  | 來源服務代碼  | 固定值：011006       |
...
| APPLY_TYPE      | varchar(10)   | 申請類別       | 3:換發(更名), 4:換發(污損) |
...
**路由**：`/Apply_011006/Prompt`

**功能說明**：顯示申請前的說明事項

**程式碼位置**：Apply_011006Controller.cs (20-32 行)
...
**路由**：`/Apply_011006/Apply?agree=1`

**功能說明**：顯示申請表單，並自動帶入會員資料

**程式碼位置**：Apply_011006Controller.cs (42-105 行)
...
4. 自動帶入電話、行動電話、Email
5. 自動帶入通訊地址
6. 設定預設值 (APPLY_TYPE = "3", MERGEYN = "N", H_EQUAL = "N")

**回傳**：View(FormModel)

### 3. Apply() POST - AJAX 驗證

**路由**：`/Apply_011006/Apply` (POST)

**功能說明**：AJAX 表單驗證

**程式碼位置**：Apply_011006Controller.cs (113-221 行)
...
4. 驗證檔案上傳：
   - 合併列印 (MERGEYN = "Y")：至少上傳 1 個檔案
   - 不合併列印 (MERGEYN = "N")：必須上傳身分證正反面 + 照片
   - 申請類別為「更名」(APPLY_TYPE = "3")：必須上傳戶籍謄本
5. 驗證檔案大小和格式
...
**路由**：`/Apply_011006/PreView` (POST)

**功能說明**：顯示申請資料預覽

**程式碼位置**：Apply_011006Controller.cs (229-234 行)
...
**回傳**：PartialView("PreView011006")

### 5. Save() POST - 儲存申請

**路由**：`/Apply_011006/Save` (POST)

**功能說明**：儲存申請資料

**程式碼位置**：Apply_011006Controller.cs (242-257 行)
...
**路由**：`/Apply_011006/AppDoc?APP_ID=xxx`

**功能說明**：顯示補件頁面

**程式碼位置**：Apply_011006Controller.cs (267-393 行)
...
**路由**：`/Apply_011006/SaveAppDoc` (POST)

**功能說明**：儲存補件資料

**程式碼位置**：Apply_011006Controller.cs (400-474 行)
...
3. 驗證表單資料
4. 開啟 Transaction
5. 更新 APPLY_011006 明細表資料
6. 上傳補件檔案並更新 APPLY_FILE 資料
7. 更新 APPLY_NOTICE 補件註記 (APPDOC_MK = "N")
8. 新增 APPLY_LOG 歷程資料
9. 寄送補件完成通知
...
**路由**：`/Apply_011006/Done?status=1`

**功能說明**：顯示申請/補件完成頁面

**程式碼位置**：Apply_011006Controller.cs (481-489 行)
...
**功能說明**：新增 APPLY_011006 明細資料

**參數**：

- `Apply_011006FormModel model`：表單資料
- `string APP_ID`：案件編號
...
**功能說明**：更新 APPLY_011006 明細資料 (補件用)

**參數**：

- `Apply_011006AppDocModel model`：補件資料
...
1. 建立 SQL SELECT 語句
2. 查詢 APPLY_011006 的檔案欄位
3. 組合檔案清單
4. 回傳 Apply_011006AppDocModel
...
**說明**：支援申請類別 (更名/污損) 和專科類別 (5 種) 的雙重選擇
...
- `ES/Controllers/Apply_011006Controller.cs` (629 行)
- `ES/Areas/BACKMIN/Controllers/Apply_011006Controller.cs` (後台審核)

### ViewModel 檔案

- `ES/Models/ViewModels/Apply_011006ViewModel.cs` (1,231 行)

### Entity Model 檔案

- `ES/Models/Entities/Apply_011006Model.cs` (163 行)

### DAO 檔案

- `ES/DataLayers/ApplyDAO.cs` (AppendApply011006, UpdateApply011006, GetFile_011006)
- `ES/DataLayers/BackApplyDAO.cs` (QueryApply_011006, CheckApply011006, AppendApply011006)

### View 檔案

- `ES/Views/Apply_011006/Prompt.cshtml` - 前導說明頁
- `ES/Views/Apply_011006/Apply.cshtml` - 申請表單頁
- `ES/Views/Apply_011006/PreView011006.cshtml` - 預覽頁面
- `ES/Views/Apply_011006/AppDoc.cshtml` - 補件頁面
- `ES/Views/Apply_011006/Done.cshtml` - 完成頁面
- `ES/Areas/BACKMIN/Views/Apply_011006/Index.cshtml` - 後台審核頁面

### JavaScript 檔案

- `ES/Scripts/Apply_011006.js` - 前端驗證和互動邏輯

### CSS 檔案

- `ES/Content/Apply_011006.css` - 樣式定義
...
**說明**：根據 MERGEYN 和 APPLY_TYPE 參數決定檔案上傳規則
...
| 2025-01-15 | 1.5.0 | 開發團隊 | 新增更名/污損申請類別 |
...
| 項目     | 011005 證書補發(遺失)  | 011006 證書換發(更名/污損) |
| -------- | ---------------------- | -------------------------- |
| 申請類別 | 1:初次, 2:補發, 3:換發 | 3:換發(更名), 4:換發(污損) |
| 戶籍謄本 | 選填                   | 更名時必填                 |
| 申請原因 | 證書遺失               | 更名或證書污損             |
| 繳費需求 | 否                     | 否                         |
| 審核重點 | 身分驗證               | 更名證明或污損證明         |
...
| 項目         | 011001 執業執照申請 | 011006 證書換發(更名/污損) |
| ------------ | ------------------- | -------------------------- |
| 服務性質     | 執業執照申請        | 證書換發                   |
| 繳費需求     | 是                  | 否                         |
| 申請期間控制 | 無                  | 無                         |
