...
| Controller   | `ES/Controllers/Apply_011009Controller.cs` (450 行)        |
| ViewModel    | `ES/Models/ViewModels/Apply_011009ViewModel.cs` (1,033 行) |
| Entity Model | `ES/Models/Entities/Apply_011009Model.cs` (153 行)         |
| 主要資料表   | APPLY, APPLY_011009, APPLY_FILE, APPLY_LOG, APPLY_NOTICE   |
| 檔案上傳數量 | 4 種                                                       |
| 補件功能     | ✅ 支援                                                    |
| Email 通知   | ✅ 支援                                                    |
| 前導說明頁   | ✅ 必須同意才能申請                                        |
...
| 功能項目         | 011009 證書補發(遺失) | 011008 證書換發(更名或汙損) | 011007 證書核發(中文) |
| ---------------- | --------------------- | --------------------------- | --------------------- |
| 服務性質         | 證書補發              | 證書換發                    | 證書核發              |
| 申請類別選擇     | ❌                    | ✅ (更名/汙損)              | ❌                    |
| 更名前姓名       | ❌                    | ✅                          | ❌                    |
...
1. 承辦人員發送補件通知
2. 系統解析需補件欄位清單 (FieldStr)
3. 前端 JavaScript 動態顯示需補件欄位
4. 申請人填寫/上傳補件資料
5. 送出補件後更新 APPDOC_MK = "N"

### 9. Email 自動通知

**說明**：申請和補件完成後自動寄送 Email
...
graph TD
    ...
    C --> D{是否同意?}
    D -->|否| C
    D -->|是| E[申請表單頁 Apply GET]
    E --> F[自動帶入會員資料]
    F --> G[填寫考試資訊]
    G --> H[填寫聯絡資訊]
    H --> I[上傳檔案]
    I --> J[AJAX 驗證 Apply POST]
    J --> K{驗證通過?}
    K -->|否| G
    K -->|是| L[預覽頁面 PreView]
    L --> M{確認送出?}
...
graph TD
    ...
    G --> H[載入案件資料]
    H --> I[載入補件通知內容]
    I --> J[解析需補件欄位 FieldStr]
    J --> K[動態顯示需補件欄位]
    K --> L[填寫/上傳補件資料]
    L --> M[儲存補件 SaveAppDoc]
    M --> N[更新 APPLY_011009]
    N --> O[上傳補件檔案]
    O --> P[更新 APPLY_NOTICE APPDOC_MK=N]
    P --> Q[新增 APPLY_LOG]
    Q --> R[寄送補件完成通知]
    R --> S[完成頁面 Done]
    S --> T[結束]
...
graph LR
    ...
    C --> D[DAO]
    D --> E[APPLY 主表]
    D --> F[APPLY_011009 明細表]
    D --> G[APPLY_FILE 檔案表]
    D --> H[APPLY_LOG 歷程表]
    D --> I[APPLY_NOTICE 通知表]
    E --> J[Email 服務]
    J --> K[申請人]
```
...
| FLOW_CD  | varchar(2)    | 案件狀態      | 1:新申請, 2:補件, 3:補件收件 |
| MAILBODY | nvarchar(max) | 補件通知內容  |                              |
| DEL_MK   | varchar(1)    | 刪除註記      | Y/N                          |

### 2. APPLY_011009 (明細表)

**功能說明**：儲存社工師證書補發（遺失）的詳細資訊
...
| C_ZIPCODE     | varchar(5)    | 通訊地址郵遞區號       |                      |
| C_ADDR        | nvarchar(300) | 通訊地址               |                      |
| H_ZIPCODE     | varchar(5)    | 戶籍地址郵遞區號       |                      |
| H_ADDR        | nvarchar(300) | 戶籍地址               |                      |
| H_EQUAL       | varchar(1)    | 同通訊地址             | Y/N                  |
| TEST_YEAR     | varchar(3)    | 考試年度               | Required             |
| TEST_CATEGORY | nvarchar(200) | 考試名稱類科           | Required             |
...
**路由**：`/Apply_011009/Prompt`

**功能說明**：顯示申請前的說明事項

**程式碼位置**：Apply_011009Controller.cs (30-42 行)
...
**路由**：`/Apply_011009/Apply?agree=1`

**功能說明**：顯示申請表單，並自動帶入會員資料

**程式碼位置**：Apply_011009Controller.cs (50-95 行)
...
**路由**：`/Apply_011009/Apply` (POST)

**功能說明**：AJAX 表單驗證

**程式碼位置**：Apply_011009Controller.cs (103-161 行)
...
**路由**：`/Apply_011009/PreView` (POST)

**功能說明**：顯示申請資料預覽

**程式碼位置**：Apply_011009Controller.cs (169-178 行)
...
**路由**：`/Apply_011009/Save` (POST)

**功能說明**：儲存申請資料

**程式碼位置**：Apply_011009Controller.cs (186-203 行)

**主要邏輯**：

1. 產生案件編號 (APP_ID)
2. 呼叫 DAO 新增 APPLY 主表資料
3. 呼叫 DAO 新增 APPLY_011009 明細表資料
...
**路由**：`/Apply_011009/AppDoc?APP_ID=xxx`

**功能說明**：顯示補件頁面

**程式碼位置**：Apply_011009Controller.cs (213-329 行)

**主要邏輯**：

1. 驗證案件編號
2. 驗證案件所有權 (申請人 = 登入者)
3. 載入案件資料 (APPLY + APPLY_011009)
...
**路由**：`/Apply_011009/SaveAppDoc` (POST)

**功能說明**：儲存補件資料

**程式碼位置**：Apply_011009Controller.cs (336-355 行)

**主要邏輯**：

1. 驗證案件編號
2. 驗證案件所有權
3. 呼叫 DAO 更新 APPLY_011009 明細表資料
...
**路由**：`/Apply_011009/Done?status=1`

**功能說明**：顯示申請/補件完成頁面

**程式碼位置**：Apply_011009Controller.cs (362-370 行)
...
**功能說明**：新增 APPLY_011009 明細資料

**參數**：

- `Apply_011009FormModel model`：表單資料
- `string APP_ID`：案件編號
...
**功能說明**：更新 APPLY_011009 明細資料 (補件用)

**參數**：

- `Apply_011009AppDocModel model`：補件資料
...
1. 建立 SQL SELECT 語句
2. 查詢 APPLY_011009 的檔案欄位
3. 組合檔案清單
4. 回傳 Apply_011009AppDocModel
...
- `ES/Controllers/Apply_011009Controller.cs` (450 行)
- `ES/Areas/BACKMIN/Controllers/Apply_011009Controller.cs` (後台審核)

### ViewModel 檔案

- `ES/Models/ViewModels/Apply_011009ViewModel.cs` (1,033 行)

### Entity Model 檔案

- `ES/Models/Entities/Apply_011009Model.cs` (153 行)

### DAO 檔案

- `ES/DataLayers/ApplyDAO.cs` (AppendApply011009, UpdateApply011009, GetFile_011009)
- `ES/DataLayers/BackApplyDAO.cs` (QueryApply_011009, CheckApply011009)

### View 檔案

- `ES/Views/Apply_011009/Prompt.cshtml` - 前導說明頁
- `ES/Views/Apply_011009/Index.cshtml` - 申請表單頁
- `ES/Views/Apply_011009/PreView.cshtml` - 預覽頁面
- `ES/Views/Apply_011009/AppDoc.cshtml` - 補件頁面
- `ES/Views/Apply_011009/Done.cshtml` - 完成頁面
- `ES/Areas/BACKMIN/Views/Apply_011009/Index.cshtml` - 後台審核頁面

### JavaScript 檔案

- `ES/Scripts/Apply_011009.js` - 前端驗證和互動邏輯

### CSS 檔案

- `ES/Content/Apply_011009.css` - 樣式定義
...
| 2025-01-15 | 1.4.0 | 開發團隊 | 優化 Email 通知功能 |
...
- [011008 社工師證書換發(更名或汙損) - 完整技術文件](<./011008_社工師證書換發(更名或汙損)_完整技術文件.md>)
...
### 與 011008 (證書換發-更名或汙損) 的差異

| 項目         | 011008 證書換發(更名或汙損) | 011009 證書補發(遺失) |
| ------------ | --------------------------- | --------------------- |
| 服務性質     | 證書換發                    | 證書補發              |
| 申請類別     | ✅ (更名/汙損)              | ❌                    |
| 更名前姓名   | ✅                          | ❌                    |
...
| 項目         | 011007 證書核發(中文) | 011009 證書補發(遺失) |
| ------------ | --------------------- | --------------------- |
| 服務性質     | 證書核發              | 證書補發              |
| 申請類別     | ❌                    | ❌                    |
| 更名前姓名   | ❌                    | ❌                    |
