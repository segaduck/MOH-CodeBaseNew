...
namespace ES.Controllers
{
    public class Apply_011009Controller : BaseController
    {
        ...
        public static string s_SRV_ID = "011009";//011002
        public static string s_SRV_NAME = "社工師證書補發（遺失）";
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult Apply(Apply_011009FormModel model)
        {
            SessionModel sm = SessionModel.Get();
            if (sm == null || sm.UserInfo == null) { return RedirectToAction("Index", "History"); }
            ClamMember mem = sm.UserInfo.Member;
            ...
            {
                logger.Debug("Apply_011009.FileName:" + model.FILE_IDNF_TEXT);
                // 允許的附檔名（全小寫）
            ...
            {
                logger.Debug("Apply_011009.FileName:" + model.FILE_IDNB_TEXT);
                // 允許的附檔名（全小寫）
                var validExts = new[] { "pdf", "jpg", "jpeg", "bmp", "png", "gif", "tif", "zip", "doc", "docx", "odt", "odf", "ods", "xls", "xlsx", "ppt", "pptx" };
                // 取得副檔名並轉小寫
            ...
            {
                logger.Debug("Apply_011009.FileName:" + model.FILE_PHOTO_TEXT);
                // 允許的附檔名（全小寫）
                var validExts = new[] { "pdf", "jpg", "jpeg", "bmp", "png", "gif", "tif", "zip", "doc", "docx", "odt", "odf", "ods", "xls", "xlsx", "ppt", "pptx" };
                // 取得副檔名並轉小寫
            ...
            {
                logger.Debug("Apply_011009.FileName:" + model.FILE_PASSCOPY_TEXT);
                // 允許的附檔名（全小寫）
                var validExts = new[] { "pdf", "jpg", "jpeg", "bmp", "png", "gif", "tif", "zip", "doc", "docx", "odt", "odf", "ods", "xls", "xlsx", "ppt", "pptx" };
                // 取得副檔名並轉小寫
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult PreView(Apply_011009FormModel model)
        {
            //ApplyDAO dao = new ApplyDAO();
            SessionModel sm = SessionModel.Get();
            if (sm == null || sm.UserInfo == null) { return RedirectToAction("Index", "History"); }
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult Save(Apply_011009FormModel model)
        {
            SessionModel sm = SessionModel.Get();
            if (sm == null || sm.UserInfo == null) { return RedirectToAction("Index", "History"); }
            ClamMember mem = sm.UserInfo.Member;
            ...
            string APP_ID = dao.GetApp_ID(s_SRV_ID);// "011009");
            // 存檔
            dao.AppendApply011009(model, APP_ID);
            // 寄信
        ...
        public ActionResult AppDoc(string APP_ID)
        {
            ...
            ApplyDAO dao = new ApplyDAO();
            Apply_011009AppDocModel model = new Apply_011009AppDocModel();
            //Apply_011009DetailModel detail = new Apply_011009DetailModel();

            // 案件基本資訊
            Apply_011009Model app_SN_Where = new Apply_011009Model();
            app_SN_Where.APP_ID = APP_ID;
            Apply_011009Model app_SN_data = dao.GetRow(app_SN_Where);
            ...
            model = dao.GetFile_011009(APP_ID);
            ...
            ////專科類別
            //model.SPECIALIST_TYPE = app_SN_data.SPECIALIST_TYPE;
            // 姓名
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public ActionResult SaveAppDoc(Apply_011009AppDocModel model)
        {
            SessionModel sm = SessionModel.Get();
            if (sm == null || sm.UserInfo == null) { return RedirectToAction("Index", "History"); }
            ClamMember mem = sm.UserInfo.Member;
            ...
            // 存檔
            var count = dao.UpdateApply011009(model);
            // 寄信
        ...
        {
            SessionModel sm = SessionModel.Get();
            Apply_011009DoneModel model = new Apply_011009DoneModel();
        ...
        //[HttpPost]
        //[DisplayName("011009_申請單套表")]
        //public void PreviewApplyForm(Apply_011009FormModel Form)
        //{
        //    //□ ■
        //    string path = Server.MapPath("~/Sample/apply011009.docx");
        //    byte[] buffer = null;
