...
using ES.Utils;
using HppApi;
//using ES.Areas.BACKMIN.Views.Account;
...
namespace ES.Controllers
{
    public class Apply_001005Controller : BaseController
    {
        ...
        [HttpPost]
        public ActionResult ApplyAjax(Apply_001005ViewModel view)
        {
            var result = new AjaxResultStruct();
            if (view.ATTACH_FILE_NAME != null)
            {
                logger.Debug("Apply_001005.FileName:" + view.ATTACH_FILE_NAME);
                // 允許的附檔名（全小寫）
        ...
        public ActionResult Save(Apply_001005ViewModel model)
        {
            ...
            switch (model.PAY_METHOD)
            {
                //信用卡線上刷卡（電子化政府網路付費服務）
                case "C":
                    //ApplyModel apply = dao.GetNewApplyEmptyRow("001005", 4);
                    ...
                    {
                        dao.SendMail_Proc(memberName, memberEmail, model.APP_ID, "醫事人員或公共衛生師證書補(換)發", "001005");
                    }
                    break;
                //匯票(抬頭：衛生福利部）
                case "D":
        ...
        public ActionResult AppDoc(string APP_ID)
        {
            ...
            model = new Apply_001005ViewModel();
            model.APP_ID = APP_ID;
            ...
            try
            {
                ...
                if (applyData.ACC_NO == userInfo.ACC_NO)
                {
                    ...
                    model.IsNotice = "Y";
                    if (applyData.FLOW_CD == "2" && shareDAO.CalculationDocDate("001005", model.APP_ID))
                    {
        ...
        public ActionResult AppDocSave(Apply_001005ViewModel model)
        {
            ...
            try
            {
                ...
                {
                    dao.SendMail_Update(memberName, memberEmail, model.APP_ID, "醫事人員或公共衛生師證書補（換）發", "001005", "-1");
