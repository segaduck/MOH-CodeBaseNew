...
namespace ES.Controllers
{
    public class Apply_011007Controller : BaseController
    {
        ...
        public static string s_SRV_ID = "011007";//011002
        public static string s_SRV_NAME = "社工師證書核發（中文）";//"專科社會工作師證書核發"
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult Apply(Apply_011007FormModel model)
        {
            SessionModel sm = SessionModel.Get();
            if (sm == null || sm.UserInfo == null) { return RedirectToAction("Index", "History"); }
            ClamMember mem = sm.UserInfo.Member;
            ...
            {
                logger.Debug("Apply_011007.FileName:" + model.FILE_IDNF_TEXT);
                // 允許的附檔名（全小寫）
            ...
            {
                logger.Debug("Apply_011007.FileName:" + model.FILE_IDNB_TEXT);
                // 允許的附檔名（全小寫）
                var validExts = new[] { "pdf", "jpg", "jpeg", "bmp", "png", "gif", "tif", "zip", "doc", "docx", "odt", "odf", "ods", "xls", "xlsx", "ppt", "pptx" };
                // 取得副檔名並轉小寫
            ...
            {
                logger.Debug("Apply_011007.FileName:" + model.FILE_PHOTO_TEXT);
                // 允許的附檔名（全小寫）
                var validExts = new[] { "pdf", "jpg", "jpeg", "bmp", "png", "gif", "tif", "zip", "doc", "docx", "odt", "odf", "ods", "xls", "xlsx", "ppt", "pptx" };
                // 取得副檔名並轉小寫
            ...
            {
                logger.Debug("Apply_011007.FileName:" + model.FILE_PASSCOPY_TEXT);
                // 允許的附檔名（全小寫）
                var validExts = new[] { "pdf", "jpg", "jpeg", "bmp", "png", "gif", "tif", "zip", "doc", "docx", "odt", "odf", "ods", "xls", "xlsx", "ppt", "pptx" };
                // 取得副檔名並轉小寫
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult PreView(Apply_011007FormModel model)
        {
            SessionModel sm = SessionModel.Get();
            if (sm == null || sm.UserInfo == null) { return RedirectToAction("Index", "History"); }
            ClamMember mem = sm.UserInfo.Member;
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult Save(Apply_011007FormModel model)
        {
            SessionModel sm = SessionModel.Get();
            if (sm == null || sm.UserInfo == null) { return RedirectToAction("Index", "History"); }
            ClamMember mem = sm.UserInfo.Member;
            ...
            string APP_ID = dao.GetApp_ID(s_SRV_ID);// "011007");
            // 存檔
            dao.AppendApply011007(model, APP_ID);
            // 寄信
        ...
        public ActionResult AppDoc(string APP_ID)
        {
            ...
            Apply_011007Model app_SN_data = dao.GetRow(app_SN_Where);
            ...
            Apply_011007AppDocModel model = new Apply_011007AppDocModel();
            // 取檔案(可依個人方式決定帶值回來的方式，建議用SQL)
            model = dao.GetFile_011007(APP_ID);
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public ActionResult SaveAppDoc(Apply_011007AppDocModel model)
        {
            SessionModel sm = SessionModel.Get();
            if (sm == null || sm.UserInfo == null) { return RedirectToAction("Index", "History"); }
            ClamMember mem = sm.UserInfo.Member;
            ...
            // 存檔
            var count = dao.UpdateApply011007(model);
            // 寄信
        ...
        public ActionResult Done(string status, string Count = "")
        {
            ...
            Apply_011007DoneModel model = new Apply_011007DoneModel();
        ...
        //[HttpPost]
        //[DisplayName("011007_申請單套表")]
        //public void PreviewApplyForm(Apply_011007FormModel Form)
        //{
        //    //□ ■
        //    string path = Server.MapPath("~/Sample/apply011007.docx");
        //    byte[] buffer = null;
