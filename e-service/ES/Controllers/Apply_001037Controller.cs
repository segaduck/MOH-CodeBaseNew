...
namespace ES.Controllers
{
    ...
    {
        public static string s_SRV_ID = "001037";
        public static string s_SRV_NAME = "醫事人員或公共衛生師請領無懲戒紀錄證明申請書";
        ...
        public ActionResult Index(Apply_001037ViewModel model)
        {
            return View();
        }
        ...
        [HttpPost]
        public ActionResult ApplyAjax(Apply_001037ViewModel view)
        {
            var result = new AjaxResultStruct();
            if (view.FILE1_NAME != null)
            {
                logger.Debug("Apply_001037.FileName:" + view.FILE1_NAME);
                // 允許的附檔名（全小寫）
            ...
            {
                logger.Debug("Apply_001037.FileName:" + view.FILE2_NAME);
                // 允許的附檔名（全小寫）
                var validExts = new[] { "pdf", "jpg", "jpeg", "bmp", "png", "gif", "tif", "zip" };
                // 取得副檔名並轉小寫
            ...
            {
                logger.Debug("Apply_001037.FileName:" + view.FILE3_NAME);
                // 允許的附檔名（全小寫）
                var validExts = new[] { "pdf", "jpg", "jpeg", "bmp", "png", "gif", "tif", "zip" };
                // 取得副檔名並轉小寫
        ...
        public ActionResult Save(Apply_001037ViewModel model)
        {
            ...
            switch (model.PAY_METHOD)
            {
                //信用卡線上刷卡（電子化政府網路付費服務）
                case "C":
                    //ApplyModel apply = dao.GetNewApplyEmptyRow("001037", 4);
        ...
        public ActionResult AppDocSave(Apply_001037ViewModel model)
        {
            ...
            try
            {
                ...
                {
                    dao.SendMail_Update(sm.UserInfo.Member.NAME, sm.UserInfo.Member.MAIL, model.APP_ID, "醫事人員或公共衛生師請領無懲戒紀錄證明申請書", "001037", "-1");
