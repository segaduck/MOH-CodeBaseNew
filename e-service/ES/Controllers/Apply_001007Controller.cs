...
namespace ES.Controllers
{
    public class Apply_001007Controller : BaseController
    {
        ...
        [HttpPost]
        public ActionResult ApplyAjax(Apply_001007ViewModel view)
        {
            var result = new AjaxResultStruct();
            if (view.ATTACH_FILE_NAME != null)
            {
                logger.Debug("Apply_001007.FileName:" + view.ATTACH_FILE_NAME);
                // 允許的附檔名（全小寫）
        ...
        public ActionResult Save(Apply_001007ViewModel model)
        {
            ...
            switch (model.PAY_METHOD)
            {
                //信用卡線上刷卡（電子化政府網路付費服務）
                case "C":
                    //ApplyModel apply = dao.GetNewApplyEmptyRow("001007", 4);
                    ...
                    {
                        dao.SendMail_Proc(memberName, memberEmail, model.APP_ID, "專科醫師證書補（換）發", "001007");
                    }
                    break;
                //匯票(抬頭：衛生福利部）
                case "D":
        ...
        public ActionResult AppDoc(string APP_ID)
        {
            ...
            model = new Apply_001007ViewModel();
            model.APP_ID = APP_ID;
            ...
            try
            {
                ...
                if (applyData.ACC_NO == userInfo.ACC_NO)
                {
                    ...
                    model.IsNotice = "Y";
                    if (applyData.FLOW_CD == "2" && shareDAO.CalculationDocDate("001007", model.APP_ID))
                    {
        ...
        public ActionResult AppDocSave(Apply_001007ViewModel model)
        {
            ...
            try
            {
                ...
                {
                    dao.SendMail_Update(memberName, sm.UserInfo.Member.MAIL, model.APP_ID, "專科醫師證書補（換）發", "001007", "-1");
            ...
            {
                logger.Error("Apply_001007_補件存檔__AppDocSave failed:" + ex.TONotNullString());
                sm.LastErrorMessage = "存檔失敗!!";
            }
