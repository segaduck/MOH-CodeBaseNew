...
namespace ES.Controllers
{
    ...
    /// 專科社會工作師證書補發（遺失）
    /// </summary>
    public class Apply_011005Controller : BaseController
    {
        ...
        public ActionResult Prompt()
        {
            ...
            Apply_011005ViewModel model = new Apply_011005ViewModel();
            //ActionResult rtn = View("Prompt", model.Form);
            string s_msg_1A = "請先閱讀 「{0}說明事項」點選同意後，再進入申辦頁面 !";
            sm.LastErrorMessage = string.Format(s_msg_1A, "專科社會工作師證書補發（遺失）");
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult Apply(Apply_011005FormModel model)
        {
            ...
            {
                logger.Debug("Apply_011005.FileName:" + model.FILE_IDNF_TEXT);
                // 允許的附檔名（全小寫）
            ...
            {
                logger.Debug("Apply_011005.FileName:" + model.FILE_IDNB_TEXT);
                // 允許的附檔名（全小寫）
                var validExts = new[] { "pdf", "jpg", "jpeg", "bmp", "png", "gif", "tif", "zip", "doc", "docx", "odt", "odf", "ods", "xls", "xlsx", "ppt", "pptx" };
                // 取得副檔名並轉小寫
            ...
            {
                logger.Debug("Apply_011005.FileName:" + model.FILE_PHOTO_TEXT);
                // 允許的附檔名（全小寫）
                var validExts = new[] { "pdf", "jpg", "jpeg", "bmp", "png", "gif", "tif", "zip", "doc", "docx", "odt", "odf", "ods", "xls", "xlsx", "ppt", "pptx" };
                // 取得副檔名並轉小寫
            ...
            {
                logger.Debug("Apply_011005.FileName:" + model.FILE_HOUSEHOLD_TEXT);
                // 允許的附檔名（全小寫）
                var validExts = new[] { "pdf", "jpg", "jpeg", "bmp", "png", "gif", "tif", "zip", "doc", "docx", "odt", "odf", "ods", "xls", "xlsx", "ppt", "pptx" };
                // 取得副檔名並轉小寫
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult PreView(Apply_011005FormModel model)
        {
            ApplyDAO dao = new ApplyDAO();

            return PartialView("PreView011005", model);
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult Save(Apply_011005FormModel model)
        {
            SessionModel sm = SessionModel.Get();
            ApplyDAO dao = new ApplyDAO();
            ...
            var APP_ID = dao.GetApp_ID("011005");
            // 存檔
            dao.AppendApply011005(model, APP_ID);
            // 寄信
            dao.SendMail_New(memberName, memberEmail, APP_ID, "專科社會工作師證書補發（遺失）", "011005", ISSEND: true);
        ...
        public ActionResult AppDoc(string APP_ID)
        {
            ApplyDAO dao = new ApplyDAO();
            SessionModel sm = SessionModel.Get();
            Apply_011005AppDocModel model = new Apply_011005AppDocModel();
            //Apply_011005DetailModel detail = new Apply_011005DetailModel();

            // 案件基本資訊
            Apply_011005Model app_011005 = new Apply_011005Model();
            app_011005.APP_ID = APP_ID;
            var app_011005data = dao.GetRow(app_011005);
            ...
            try
            {
                ...
                if (appdata.ACC_NO == UsIn.ACC_NO)
                {
                    ...
                    model = dao.GetFile_011005(APP_ID);
                    ...
                    //申請用途
                    model.APPLY_TYPE = app_011005data.APPLY_TYPE;
                    //專科類別
                    model.SPECIALIST_TYPE = app_011005data.SPECIALIST_TYPE;
                    // 姓名
                    ...
                    // 電話
                    model.W_TEL = app_011005data.W_TEL;
                    if (!string.IsNullOrEmpty(app_011005data.W_TEL))
                    {
                        model.W_TEL_0 = app_011005data.W_TEL.ToSplit('-').FirstOrDefault();
                        model.W_TEL_1 = app_011005data.W_TEL.ToSplit('-').LastOrDefault().ToSplit('#').FirstOrDefault();
                        if (model.W_TEL.IndexOf('#') > 0)
                        {
                            model.W_TEL_2 = app_011005data.W_TEL.ToSplit('#').LastOrDefault();
                        }
                    }
                    model.H_TEL = app_011005data.H_TEL;
                    if (!string.IsNullOrEmpty(app_011005data.H_TEL))
                    {
                        model.H_TEL_0 = app_011005data.H_TEL.ToSplit('-').FirstOrDefault();
                        model.H_TEL_1 = app_011005data.H_TEL.ToSplit('-').LastOrDefault().ToSplit('#').FirstOrDefault();
                        if (model.H_TEL.IndexOf('#') > 0)
                        {
                            model.H_TEL_2 = app_011005data.H_TEL.ToSplit('#').LastOrDefault();
                        }
                    }
                    //手機
                    model.MOBILE = appdata.MOBILE;
                    // 地址
                    model.C_ZIPCODE = app_011005data.C_ZIPCODE;
                    model.C_ADDR = app_011005data.C_ADDR;
                    model.H_ZIPCODE = app_011005data.H_ZIPCODE;
                    model.H_ADDR = app_011005data.H_ADDR;
                    model.H_EQUAL = app_011005data.H_EQUAL;
                    // 行動
                    model.MOBILE = appdata.MOBILE;
                    // Mail
                    model.EMAIL = app_011005data.EMAIL;
                    // 執業處所
                    model.PRACTICE_PLACE = app_011005data.PRACTICE_PLACE;
                    //考試年度
                    model.TEST_YEAR = app_011005data.TEST_YEAR;
                    //合併上傳
                    model.MERGEYN = app_011005data.MERGEYN;
                    #endregion
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public ActionResult SaveAppDoc(Apply_011005AppDocModel model)
        {
            SessionModel sm = SessionModel.Get();
            ApplyDAO dao = new ApplyDAO();
            var memberName = string.IsNullOrWhiteSpace(model.NAME) ? sm.UserInfo.Member.NAME : model.NAME;
            ...
            if (ModelState.IsValid)
            {
                ModelState.Clear();
                // 存檔
                var count = dao.UpdateApply011005(model);
                // 寄信
                dao.SendMail_Update(memberName, memberEmail, model.APP_ID, "專科社會工作師證書補發（遺失）", "011005", count, ISSEND: true);
                return Done("2", count);
            }
            else
            {
        ...
        {
            SessionModel sm = SessionModel.Get();
            Apply_011005DoneModel model = new Apply_011005DoneModel();
        ...
        #region 申請單套表
        //[HttpPost]
        //[DisplayName("011005_申請單套表")]
        //public void PreviewApplyForm(Apply_011005FormModel Form)
        //{
        //    //□ ■
        //    string path = Server.MapPath("~/Sample/apply011005.docx");
        //    byte[] buffer = null;
