...
//formSelector: HTML form 元素的 CSS 選擇子表達式（例："#main_form"）或是 jquery 物件（例：$("#main_form")）。
//jsonExtra:（非必要）額外加入的自訂資料 JSON 物件（輸入 null 表示不需要）。
function getDivDataJson(formSelector, jsonExtra) {
    var ret = {};
    if (formSelector !== undefined && formSelector != null) {
        var form = $(formSelector);
        //debugger;
        if (form.length > 0) {
            try {
                var raw = form.find('select, textarea, input').serialize();
                if (raw.length > 0) {
                    var nv, pairs = raw.split("&");
                    $.each(pairs, function (idx, text) {
                        nv = text.split("=");
...
function popupWindow(windowConfig, contentUrl, title, jsonParam, closeCallback, loadCallback, closeConfirmCallback, shownCallback) {
    try {
        ...
        //     "        <div class=\"modal-body\" data-id=\"", divId, "\" style=\"overflow:none;width:", wdW, "px;height:", wdH, "px;max-height:100%;\">",
        //     "           <div id=\"loading\" class=\"text-center\">",
        //     "               <img src=\"", wdLoadingGif, "\" width=\"100\" height=\"100\" alt=\"頁面載入中，請稍候...\" />",
        //     "           </div>",
        //     "           <iframe id=\"", ifrId, "\" name=\"", ifrId, "\" src=\"", ((wdMethod == "GET") ? wdUrl : ""), "\" frameborder=\"0\" style=\"width:100%;height:100%;overflow:auto;display:none;\"></iframe>",
        //     "        </div>",
        //     "        <div class=\"modal-footer\" style=\"height:35px;padding:6px 10px 6px 10px;cursor:move;\">",
        ...
        //     "  </div>"
        // ];
        // 2022.02.10 Add 取消計算寬度改成用樣式覆蓋
        var html = [
            "  <div class=\"modal in modal-resize modal-drag\" tabindex=\"-1\" id=\"", divId, "\" role=\"dialog\" aria-labelledby=\"popupWindow\" aria-hidden=\"true\">",
            "    <form id=\"", frmId, "\" style=\"display:none;\"  method=\"POST\" target=\"", ifrId, "\" action=\"", ((wdMethod == "POST") ? wdUrl : "") ,"\"></form>",
            "    <div class=\"modal-dialog\">",
            "      <div class=\"modal-content\">",
            ...
            "       </div>",
            "        <div class=\"modal-body\" data-id=\"", divId, "\">",
            "           <div id=\"loading\" class=\"text-center\">",
            "               <img src=\"", wdLoadingGif, "\" width=\"100\" height=\"100\" alt=\"頁面載入中，請稍候...\" />",
            "           </div>",
            "           <iframe id=\"", ifrId, "\" name=\"", ifrId, "\" src=\"", ((wdMethod == "GET") ? wdUrl : ""), "\" frameborder=\"0\"></iframe>",
            "        </div>",
...
//傳回在表單內可被提交到後端的資料輸入欄位「名稱:值」JSON 物件
//checkbox 勾選抓取
function getFormSubmitableFieldsWithCheckbox(formSelector) {
    var ret = {};
    var elms = $(formSelector).find(":input");
    var checkValue = {};
    if (elms.length > 0) {
        ...
        elms.each(function () {
            elm = $(this);
            elmName = elm.attr("name");
            if (elmName != undefined && elmName.length > 0) {
                if (elm.attr("type") == "checkbox") {
                    ret[elmName] = elm.prop("checked").toString();
                    checkValue[elmName] = elm.prop("checked").toString();
                } else {
                    ret[elmName] = elm.val();
                }
            }
    ...
    for (var i = 0; i < Object.getOwnPropertyNames(checkValue).length; i++) {
        ret[Object.getOwnPropertyNames(checkValue)[i]] = checkValue[Object.getOwnPropertyNames(checkValue)[i]];
    }
