...
namespace ES.Areas.Admin.Action
{
    public class MessageAction : BaseAction
    {
        ...
        /// <returns></returns>
        public List<MessageModel> GetList(MessageActionModel model)
        {
            List<MessageModel> list = new List<MessageModel>();

            StringBuilder countSQL = new StringBuilder(@"SELECT COUNT(1) FROM MESSAGE WHERE DEL_MK = 'N' ");
    ...
    SELECT TOP @END_COUNT MSG_ID FROM MESSAGE
    WHERE DEL_MK = 'N' ");
            ...
            using (SqlDataReader dr = cmd2.ExecuteReader())
            {
                while (dr.Read())
                {
                    ...
                    {
                        MessageID = DataUtils.GetDBInt(dr, 0),
        ...
        /// <returns></returns>
        public MessageEditModel GetData(int messageId)
        {
            MessageEditModel item = new MessageEditModel();
    ...
    AND MSG_ID = @MSG_ID";
        ...
        public string GetDataNewID()
        {
            ...
            SqlCommand cmd = new SqlCommand(sql, conn);
            DataUtils.AddParameters(cmd, "MSG_ID", "0");
            using (SqlDataReader dr = cmd.ExecuteReader())
            {
                if (dr.Read())
                {
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public bool Insert(MessageEditModel model)
        {
            string sql = @" INSERT INTO MESSAGE (
                                ...
                                ISNULL((SELECT MAX(MSG_ID) FROM MESSAGE), 0)+1,
            ...
            DataUtils.AddParameters(cmd, "SEND_MAIL_MK", (model.SendMailMark ? "Y" : "N"));
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public bool Update(MessageEditModel model)
        {
            string sql = @" UPDATE MESSAGE SET
                            ...
                            UPD_FUN_CD = @FUN_CD,
                            UPD_ACC = @UPD_ACC";
            if (model.FileName1 != null) sql += @" ,FILENAME_1 = @FILENAME_1";
            if (model.FileName2 != null) sql += @" ,FILENAME_2 = @FILENAME_2";
            if (model.FileName3 != null) sql += @" ,FILENAME_3 = @FILENAME_3";

            sql += @" WHERE MSG_ID = @MSG_ID";
            ...
            if (model.FileName1 != null) DataUtils.AddParameters(cmd, "FILENAME_1", model.FileName1);
            if (model.FileName2 != null) DataUtils.AddParameters(cmd, "FILENAME_2", model.FileName2);
            if (model.FileName3 != null) DataUtils.AddParameters(cmd, "FILENAME_3", model.FileName3);
            DataUtils.AddParameters(cmd, "SEND_MAIL_MK", (model.SendMailMark ? "Y" : "N"));
            ...
            DataUtils.AddParameters(cmd, "FUN_CD", "ADM-MSG");
            DataUtils.AddParameters(cmd, "UPD_ACC", model.UpdateAccount);
            DataUtils.AddParameters(cmd, "MSG_ID", model.MessageID);
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public bool Delete(MessageActionModel model)
        {
            string sql = @" UPDATE MESSAGE SET
    DEL_MK = 'Y',
    DEL_TIME = GETDATE(),
