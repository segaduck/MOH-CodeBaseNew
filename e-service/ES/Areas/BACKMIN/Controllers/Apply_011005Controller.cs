...
namespace ES.Areas.Admin.Controllers
{
    public class Apply_011005Controller : BaseController
    {
        ...
        public ActionResult Index(string appid, string srvid)
        {
            ...
            model.Form = new Apply_011005FormModel();
            model.Form.APP_ID = appid;
            model.Form = dao.QueryApply_011005(model.Form);
            DataTable dtPay = dao.QueryPayInfo_011005(appid);
            ...
            if (model.Form != null)
            {
                ...
                {
                    model.Detail = dao.GetApplyNotice_011005(appid);
                ...
                // 取檔案
                model.Form.FileList = dao.GetFileList_011005(model.Form.APP_ID);
            }
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult Save(Apply_011005ViewModel model)
        {
            SessionModel sm = SessionModel.Get();
            BackApplyDAO dao = new BackApplyDAO();
            ...
            string ErrorMsg = "";
            // 檢核
            ErrorMsg = dao.CheckApply011005(model);
            if (ErrorMsg == "")
            {
                ErrorMsg = dao.AppendApply011005(model);
                if (ErrorMsg == "")
                {
                    result.status = true;
                    result.message = "存檔成功 !";
        ...
        public ActionResult UpdatePayType(string APP_ID, DateTime? date, bool IS_PAY_STATUS, int PAY_A_FEE)
        {
            ...
            else
            {
                if (IS_PAY_STATUS)
                {
                    ...
                    {
                        // 檢核
                        ErrorMsg = dao.UpdatePayInfo_011002(APP_ID, IS_PAY_STATUS, date, PAY_A_FEE, "011005");
                    }
                }
            }
