...
{
    public class Apply_011002Controller : BaseController
    {
        ...
        public ActionResult Index(string appid, string srvid)
        {
            ...
            model.Form = new Apply_011002FormModel();
            model.Form.APP_ID = appid;
            model.Form = dao.QueryApply_011002(model.Form);
            DataTable dtPay = dao.QueryPayInfo_011002(appid);
            ...
            if (model.Form != null)
            {
                ...
                {
                    model.Detail = dao.GetApplyNotice_011002(appid);
                ...
                // 取檔案
                model.Form.FileList = dao.GetFileList_011002(model.Form.APP_ID);
            }
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult Save(Apply_011002ViewModel model)
        {
            SessionModel sm = SessionModel.Get();
            BackApplyDAO dao = new BackApplyDAO();
            ...
            string ErrorMsg = "";
            // 檢核
            ErrorMsg = dao.CheckApply011002(model);
            if (ErrorMsg == "")
            {
                ErrorMsg = dao.AppendApply011002(model);
                if (ErrorMsg == "")
                {
                    result.status = true;
                    result.message = "存檔成功 !";
        ...
        public ActionResult UpdatePayType(string APP_ID, DateTime? date, bool IS_PAY_STATUS, int PAY_A_FEE)
        {
            ...
            else
            {
                if (IS_PAY_STATUS)
                {
                    ...
                    {
                        // 檢核
                        ErrorMsg = dao.UpdatePayInfo_011002(APP_ID, IS_PAY_STATUS, date, PAY_A_FEE, "011002");
                    }
                }
            }
