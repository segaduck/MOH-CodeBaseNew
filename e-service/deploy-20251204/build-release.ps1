...
$solutionPath = Join-Path $projectRoot "ES.sln"
$projectPath = Join-Path $projectRoot "ES\ES.csproj"
...
try {
    ...
    & $msbuildPath $projectPath /p:Configuration=$Configuration /p:DeployOnBuild=true `
        /p:PublishProfile=FolderProfile /p:publishUrl=$OutputPath /p:DeleteExistingFiles=True /v:minimal
